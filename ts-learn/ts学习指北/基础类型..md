...
TypeScript支持Javascript所有的数据类型，除此之外还提供其他类型提供我们使用
...
|  数组类型  |    无     | 声明变量为数组<br />1. 在元素后面加上[]<br />2. 使用数组泛型 `let arr: Array<number> = [1, 2]` |
|    元组    |    无     | 元组类型用来表示已知元素数量和类型的数组，各元素的类型不必相同，对应位置的类型需要相同<br />`let x: [string, number]; x = ['shiyuq', 1]; // 运行正常 x = [1, 'shiyuq'] // 报错` |
|    枚举    |   enum    | 枚举类型用于定义数值集合<br />`enum Color {Red, Green, Blue}; let c: Color = Color.Blue; // 2` |
|    void    |   void    |        用于标识方法返回值的类型，表示该方法没有返回值        |
|    null    |   null    |                        标识对象值缺失                        |
| undefined  | undefined |                  用户初始化一个未定义的变量                  |
